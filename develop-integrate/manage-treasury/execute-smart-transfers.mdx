...
To cancel a ticket, call this [endpoint](/reference/cancelticket). Canceling is only possible if none of the ticket’s legs (terms) have been previously funded. Otherwise, only the side that already funded the ticket has the ability to cancel it.
